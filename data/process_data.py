...
def select_features(df):
    ...
    # select features
    selected_features = list(coefs_df['fields'][coefs_df['abs_coefs'] > 0.1])
    
    return selected_features

def merge_data_for_prediction_model(df_text, df_numeric, df_target):
    
    df_text = df_text.fillna('')
    
    df_merged = pd.merge(df_text, df_numeric, left_index=True, right_index=True)

    df_merged = pd.merge(df_merged, df_target, left_index=True, right_index=True)
    
    df_prepared = remove_missing_bycol(df_merged, ['review_scores_rating'])
...
def main():
    if len(sys.argv) == 3:
        ...
        df_numeric = df_selection[selected_features]
        text_feature = 'description'
        df_text = df[text_feature]
        target = 'review_scores_rating'
        df_target = df[target]
        df_prepared = merge_data_for_prediction_model(df_text, df_numeric, df_target)
